...
export const UserFilters = ({
  ...
  const districts = filters.searchProvince ? getDistrictsByProvince(filters.searchProvince) : [];
          ...
          <div className="flex items-center gap-2">
            <CollapsibleTrigger asChild>
              <Button variant="outline" size="sm">
                {open ? (
                  <>
                    <ChevronUp className="h-4 w-4 mr-1" /> Gizle
                  </>
                ) : (
                  <>
                    <ChevronDown className="h-4 w-4 mr-1" /> Göster
                  </>
                )}
              </Button>
            </CollapsibleTrigger>
